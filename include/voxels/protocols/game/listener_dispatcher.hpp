...
#include "listener_events.hpp"
#include "listener_responses.hpp"

#include "boost/signals2/signal.hpp"

namespace voxels::protocols::game
{
    class ListenerEventDispatcher : public EventDispatcher {
    public:
        ListenerEventDispatcher() = default;
        virtual ~ListenerEventDispatcher() = default;

        virtual responses::NewConnection operator () (const events::NewConnectionEvent& Event) = 0;
    };

    class DefaultListenerEventDispatcher final : public ListenerEventDispatcher {
    public:
        ...
        responses::NewConnection operator () (const events::NewConnectionEvent& Event) override {
            return NewConnection(Event).get();
        }
    };
}